...
function loadDateSelection(earthquakeDataFeatures) {
    ...
    // Extract decade from timestamp and count occurrences
    const counts = d3.rollup(
        earthquakeDataFeatures,
        v => v.length,
        d => {
            return Math.floor(d.properties.Year);
        },
        d => {
    ...
    function processYears() {
        ...
        // Log the years to the console
        console.log(`Submitted Start Year: ${startYear}`);
        console.log(`Submitted End Year: ${endYear}`);
        ...
        if (startYear && endYear && parseInt(startYear) <= parseInt(endYear)) {
                ...
                yr_range.push(parseFloat(year));
                }
    
            let yearMonthData = [];
            for (let year of yr_range) {  
                if (counts.has(year)) {  
                    console.log(year);
                    let monthMap = counts.get(year);  // Get the month map for the specific year
                    for (let [month, count] of monthMap.entries()) {  // Loop through the month map
                        yearMonthData.push({ year, month, count });  // Collect the data
                    }
                }
            }

            // Determine the range of years (now limited to yr_range) and define months
            const years = yr_range.sort((a, b) => a - b);  // We already know these years are from yr_range
            ...
            legendGroup.append("text")
                ...
                .text("Count");
                    ...
                    console.error("Invalid input: Start Year should be less than or equal to End Year.");
                }
    }
